...
void QImageWidget::SaveFile(QString filepath){
   ...
   // if (toolBar->isSaveBoxEnabled()&&toolBar->isrectanglingEnabled()){

 //   saveboxcontrol=false;
...

    {
        ...
        for(int i=0; i<triangles.count(); i++) {
         ...
         //   qreal propXN = (static_cast<double>(size().width() / static_cast<double>(1000)));
            ...
            out<< QString::number((triangles[i].getPoints()[0].y()), 'f', 0)+","+

                               QString::number((triangles[i].getPoints()[0].x()), 'f', 0)+","+

                               QString::number((triangles[i].getPoints()[1].y()), 'f', 0)+","+

                               QString::number((triangles[i].getPoints()[1].x()), 'f', 0)+","+

                               QString::number((triangles[i].getPoints()[2].y()), 'f', 0)+","+

                               QString::number((triangles[i].getPoints()[2].x()), 'f', 0)+"\n" ;
...

{
    ...
    //    if (toolBar->isCheckboxingEnabled()){
      ...
      //       saveboxcontrol=true;
        ...
        //      SaveFile("C:/Users/kamil/Desktop/data.txt");
          ...
          if ((toolBar->isSaveBoxEnabled()&& toolBar->isCircleEnabled())||(toolBar->isSaveBoxEnabled()&& toolBar->isrectanglingEnabled())){
...
void QImageWidget::QParseFileData(const QStringList& data) {
  ...
  for(int i =0; i <x; i++){
  //  {
   auto rect = RectToDraw(data.at(i),':',',');
   auto circle = RectToDraw(data.at(i),'/',',');

    rects.append(RectToDraw(data.at(i),':',','));
    rects.append(RectToDraw(data.at(i),':',','));
    circles.append(RectToDraw(data.at(i), '/', ','));
...
//void QImageWidget::QParseFileData1(QString filepath) {
...
//}
      ...
      foreach(TriangleToDraw triangle, triangles){
            ...
            painter.drawLines(triangle.getLines(),3);
             ...
             if (toolBar->isSaveBoxEnabled()&&toolBar->isTriangleEnabled()){
         ...
         //   saveboxcontrol=true;
            ...
            SaveFile3("C:/Users/kamil/Desktop/data.txt");

        }
    }


void QImageWidget::QParseFileData1(const QStringList& data) {
int x = data.size();
...
for(int i =0; i <x; i++){
  ...
  //  QString xy1 = data.at(i).split(';', ',');//xy1("144","81")
  //  QStringList xy2 = point.at(i).split(xySplitter);
 //   QStringList xy3 = point.at(i).split(xySplitter);

//auto rect = RectToDraw(data.at(i), split.':',',');
//auto triangle = TriangleToDraw(data.at(i),',');
//triangles.append(TriangleToDraw(data.at(i),','));

//qDebug()<<data;
//qDebug()<<triangles.append(TriangleToDraw(data.at(i),','));
// painter.drawLines(triangle.getLines(),3);
}
